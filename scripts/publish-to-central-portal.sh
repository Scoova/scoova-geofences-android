...
#   <STAGING_DIR>$STAGING_DIR/info/scoo-va/scoova-geofences-android/<version>/sdk-<version>.aar
VERSION=$(ls "$STAGING_DIR"$STAGING_DIR/info/scoo-va/scoova-geofences-android/ 2>/dev/null | sort -V | tail -1)
if [[ -z "$VERSION" ]]; then
  echo "✗ couldn't find a published version under $STAGING_DIR$STAGING_DIR/info/scoo-va/scoova-geofences-android/" >&2
  exit 3
fi

ARTIFACT_NAME="${ARTIFACT_NAME:-scoova-geofences-android-$VERSION}"
...
if [[ "$HTTP_CODE" =~ ^2[0-9][0-9]$ ]]; then
  ...
  if [[ "$PUBLISHING_TYPE" == "AUTOMATIC" ]]; then
    ...
    echo "  https://repo1.maven.org/maven2/info/scoo-va/scoova-geofences-android/$VERSION/"
